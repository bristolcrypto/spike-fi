...
void fft(uint64_t* a, size_t n, uint64_t root, uint64_t mod) {
	...
	size_t half = n / 2;
	uint64_t *even = malloc(half * sizeof(uint64_t));
	uint64_t *odd  = malloc(half * sizeof(uint64_t));
	...
	uint64_t w2 = modmul(root, root, mod);
	fft(even, half, w2, mod);
	FI_MARK( 0 );
	fft(odd, half, w2, mod);

	uint64_t w = 1;
	for (size_t j = 0; j < half; j++) {
		uint64_t t = modmul(odd[j], w, mod);
		a[j]        = (even[j] + t) % mod;
		a[j + half] = (even[j] + mod - t) % mod;
		w = modmul(w, root, mod);
	}
